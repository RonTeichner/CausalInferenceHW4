...
enablePropensityScoreTrain = False
enableMatching = False
enable_S_learner = False
...
print(f'If I use the standardScaler correcly then the std of a feature should be 1. The std of a feature: {X_train_scaled[:,50].std()}')
...
if enable_S_learner:
    ...
    nEpochs = 1000+1
    trainLoss, trainProbLoss, testLoss, testProbLoss = np.zeros(nEpochs), np.zeros(nEpochs), np.zeros(nEpochs), np.zeros(nEpochs)
    X_test_scaled, T_test, Y_test = X_test_scaled.cuda(), T_test.unsqueeze_(-1).cuda(), Y_test.cuda()
    X_train_scaled, T_train, Y_train = X_train_scaled.cuda(), T_train.unsqueeze_(-1).cuda(), Y_train.cuda()
    minTestLoss = np.inf
    for epochIdx in range(nEpochs):
...
if enable_T_learner:
    ...
    X_test_scaled_treated, T_test_treated, Y_test_treated = X_test_scaled[treatedIndexes_test].cuda(), T_test[treatedIndexes_test].unsqueeze_(-1).cuda(), Y_test[treatedIndexes_test].cuda()
    X_train_scaled_treated, T_train_treated, Y_train_treated = X_train_scaled[treatedIndexes_train].cuda(), T_train[treatedIndexes_train].unsqueeze_(-1).cuda(), Y_train[treatedIndexes_train].cuda()
    minTestLoss = np.inf
    for epochIdx in range(nEpochs):
        ...
        loss = loss_sLearner(t_recon[:, 0], Y_test_treated)
        testLoss[epochIdx] = loss.item()
    ...
    plt.xlabel('epoch')
    plt.ylabel('outcome std''s')
    plt.title('T-learner: L1-loss train dataset%d; std(Y)=%2.2f' % (datasetNum, outcomeStd))
    plt.show()
    ...
    X_test_scaled_control, T_test_control, Y_test_control = X_test_scaled[controlIndexes_test].cuda(), T_test[controlIndexes_test].unsqueeze_(-1).cuda(), Y_test[controlIndexes_test].cuda()
    X_train_scaled_control, T_train_control, Y_train_control = X_train_scaled[controlIndexes_train].cuda(), T_train[controlIndexes_train].unsqueeze_(-1).cuda(), Y_train[controlIndexes_train].cuda()
    ...
    for epochIdx in range(nEpochs):
        ...
        loss = loss_sLearner(t_recon[:, 0], Y_test_control)
        testLoss[epochIdx] = loss.item()
